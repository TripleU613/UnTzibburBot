...
impl std::fmt::Debug for Directus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Directus").field("base", &self.base.as_str()).finish()
    }
}
...
impl Filter {
    pub fn eq(field: &str, value: impl ToString) -> Self {
        Filter { field: field.into(), op: "_eq", value: value.to_string() }
    }
    pub fn in_(field: &str, values: &[String]) -> Self {
        Filter { field: field.into(), op: "_in", value: values.join(",") }
    }
}
...
impl Directus {
    ...
    async fn call<T: DeserializeOwned>(&self, method: Method, path: &str, query: &[(String, String)], body: Option<&Value>) -> Result<T> {
        let url = self.base.join(path.trim_start_matches('/'))?;
        let mut req = self.http.request(method.clone(), url.clone()).bearer_auth(&self.token).query(query);
        if let Some(b) = body {
            req = req.json(b);
        }
        let resp = req.send().await.with_context(|| format!("directus {method} {url}"))?;
        ...
        if !status.is_success() {
            ...
            return Err(DirectusError { status, message: msg, path: path.to_owned() }.into());
        }
    ...
    pub async fn list<T: DeserializeOwned>(&self, collection: &str, filters: &[Filter], limit: Option<u32>, sort: Option<&str>) -> Result<Vec<T>> {
        let mut q: Vec<(String, String)> = filters
            .iter()
            .map(|f| (format!("filter[{}][{}]", f.field, f.op), f.value.clone()))
            .collect();
        q.push(("limit".into(), limit.map(|l| l.to_string()).unwrap_or_else(|| "-1".into())));
        if let Some(s) = sort {
            q.push(("sort".into(), s.into()));
        }
        let v: Option<Vec<T>> = self.call(Method::GET, &format!("items/{collection}"), &q, None).await?;
        Ok(v.unwrap_or_default())
    }

    pub async fn first<T: DeserializeOwned>(&self, collection: &str, filters: &[Filter]) -> Result<Option<T>> {
        Ok(self.list::<T>(collection, filters, Some(1), None).await?.into_iter().next())
    }

    pub async fn get<T: DeserializeOwned>(&self, collection: &str, id: i64) -> Result<Option<T>> {
        match self.call::<Option<T>>(Method::GET, &format!("items/{collection}/{id}"), &[], None).await {
            Ok(v) => Ok(v),
            Err(e) if is_status(&e, StatusCode::FORBIDDEN) || is_status(&e, StatusCode::NOT_FOUND) => Ok(None),
            Err(e) => Err(e),
        }
    }

    pub async fn create<T: DeserializeOwned>(&self, collection: &str, item: &impl Serialize) -> Result<T> {
        let body = serde_json::to_value(item)?;
        self.call(Method::POST, &format!("items/{collection}"), &[], Some(&body)).await
    }

    pub async fn update<T: DeserializeOwned>(&self, collection: &str, id: i64, patch: &impl Serialize) -> Result<T> {
        let body = serde_json::to_value(patch)?;
        self.call(Method::PATCH, &format!("items/{collection}/{id}"), &[], Some(&body)).await
    }

    pub async fn delete(&self, collection: &str, id: i64) -> Result<()> {
        let _: Option<Value> = self.call(Method::DELETE, &format!("items/{collection}/{id}"), &[], None).await?;
        Ok(())
    }
    ...
    pub async fn delete_where(&self, collection: &str, filters: &[Filter]) -> Result<()> {
        ...
        let _: Option<Value> = self.call(Method::DELETE, &format!("items/{collection}"), &[], Some(&json!(ids))).await?;
        Ok(())
    }
    ...
    pub async fn ping(&self) -> Result<()> {
        let _: Option<Value> = self.call(Method::GET, "server/ping", &[], None).await.or_else(|e| {
            // /server/ping returns "pong" as plain text which our decoder treats as empty/null.
            if is_status(&e, StatusCode::UNAUTHORIZED) { Err(e) } else { Ok(None) }
        })?;
        Ok(())
    }

    async fn collection_exists(&self, name: &str) -> Result<bool> {
        match self.call::<Option<Value>>(Method::GET, &format!("collections/{name}"), &[], None).await {
            Ok(Some(v)) => Ok(!v.is_null()),
            Ok(None) => Ok(false),
            Err(e) if is_status(&e, StatusCode::NOT_FOUND) || is_status(&e, StatusCode::FORBIDDEN) => Ok(false),
            Err(e) => Err(e),
        }
    }
    ...
    pub async fn ensure_schema(&self, prefix: &str) -> Result<()> {
        for spec in schema(prefix) {
            ...
            let _: Option<Value> = self.call(Method::POST, "collections", &[], Some(&body)).await
            ...
            for rel in &spec.relations {
                ...
                if let Err(e) = self.call::<Option<Value>>(Method::POST, "relations", &[], Some(&rel_body)).await {
...
pub fn is_status(e: &anyhow::Error, s: StatusCode) -> bool {
    e.downcast_ref::<DirectusError>().map(|d| d.status == s).unwrap_or(false)
}
...
fn s(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "string", interface: "input", schema: json!({}), meta: json!({}) }
}
fn text(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "text", interface: "input-multiline", schema: json!({}), meta: json!({}) }
}
fn int(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "integer", interface: "input", schema: json!({}), meta: json!({}) }
}
fn bigint(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "bigInteger", interface: "input", schema: json!({}), meta: json!({}) }
}
fn boolean(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "boolean", interface: "boolean", schema: json!({"default_value": false}), meta: json!({}) }
}
fn ts(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "timestamp", interface: "datetime", schema: json!({}), meta: json!({}) }
}
fn jsonf(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "json", interface: "input-code", schema: json!({}), meta: json!({"options": {"language": "json"}}) }
}
fn fk(name: &'static str) -> FieldSpec {
    FieldSpec { name, ty: "integer", interface: "select-dropdown-m2o", schema: json!({}), meta: json!({"special": ["m2o"]}) }
}
...
fn schema(prefix: &str) -> Vec<CollectionSpec> {
    ...
    vec![
        CollectionSpec {
            ...
            fields: vec![unique(s("telegram_user_id")), s("telegram_username"), s("telegram_first_name"), jsonf("settings"), ts("created_at"), ts("updated_at")],
            relations: vec![],
        },
        CollectionSpec {
            ...
            relations: vec![RelationSpec { field: "user", related: n.users.clone() }],
        },
        CollectionSpec {
            ...
            relations: vec![RelationSpec { field: "account", related: n.accounts.clone() }],
        },
        CollectionSpec {
            ...
            relations: vec![RelationSpec { field: "conversation", related: n.conversations.clone() }],
        },
    ]
}
