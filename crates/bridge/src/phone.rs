...
mod tests {
    ...
    #[test]
    fn lists() {
        let (ok, bad) = parse_phone_list("212-736-5000, +972501234567; 415 555 0100\nabc", "US");
        assert_eq!(ok, vec!["+12127365000", "+972501234567", "+14155550100"]);
        assert_eq!(bad, vec!["abc"]);
        let (ok, _) = parse_phone_list("+12127365000 +14155550100", "US");
        assert_eq!(ok.len(), 2);
    }
}
